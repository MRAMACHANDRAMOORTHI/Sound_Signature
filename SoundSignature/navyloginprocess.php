...
include_once "db.php";
	...
	echo "<script type='text/javascript'>window.location=\"navylogin.php?result=Your Login is Locked\"</script>";
		...
		header("Location:navylogin.php?result=Your Login is Locked");
	} else {
...
*/
                ...
                $rs=mysql_query("select x1,y1,x2,y2,x3,y3,x4,y4,x5,y5,time from soundreg where name='$name' and ans$seed='$ans' and field='Navy'") or die(mysql_error());
	...
	$r=mysql_fetch_row($rs);
                    ...
                    if(((($x1<=$dbx1+3 && $x1>=$dbx1-3) && ($y1<=$dby1+3 && $y1>=$dby1-3)) || (($x2<=$dbx2+3 && $x2>=$dbx2-3) && ($y2<=$dby2+3 && $y2>=$dby2-3)) || (($x3<=$dbx3+3 && $x3>=$dbx3-3) && ($y3<=$dby3+3 && $y3>=$dby3-3)) || (($x4<=$dbx4+3 && $x4>=$dbx4-3) && ($y4<=$dby4+3 && $y4>=$dby4-3)) || (($x5<=$dbx5+3 && $x5>=$dbx5-3) && ($y5<=$dby5+3 && $y5>=$dby5-3))) && $tm==$tm1 )
                    {
                        $_SESSION['field']="Navy";
	      $_SESSION['name']=$name;
                        header("Location:navyhome.php");
                    }
                    else
                    {
		if(isset($_SESSION['ToleranceLevel'])) {
                        	$_SESSION['ToleranceLevel']++;
		echo "<script type='text/javascript'>window.location=\"navylogin.php?result=Incorrect Values\"</script>";		
		} else {
		...
		echo "<script type='text/javascript'>window.location=\"navylogin.php?result=Incorrect Values\"</script>";		
                    ...
                    header("Location:navylogin.php?result=Invalid Login");
                }
	}
/*
            }
